...
(defun all-promotion-demotion-plans-test ()
        (let ((plan '()) (start '()) (goal '()) (a '()) (b '()))
                (setf start
                        (make-operator
                                :name 'start ;;named will be used n assoc lists
		...
		(setf a
                ...
                (setf plan (make-plan
                        :operators (list start goal a b)
                        :orderings (list (cons a b) (cons b goal) (cons b goal2))
                        :links (list (make-link :from b :to goal :precond '(t i-have-a-phd))
					(make-link :from b :to goal2 :precond '(t i-have-a-phd)))
                        :start start
                        :goal goal))

                        (print "should be t in p-test:")
                        (print (threats plan start (make-link :from b :to goal :precond '(t i-have-a-phd))))
			(print "now actually testing some demotion ideas")
        		(print (all-promotion-demotion-plans plan (threats plan start (make-link :from b :to goal :precond '(t i-have-a-phd))))) 
	)
)
